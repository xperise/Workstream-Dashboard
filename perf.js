...
Object.assign(I18N.vi, {
  ...
  pFileEmpty: "File rỗng.", pReadFail: e => `Không đọc được file: ${e}`,
...
Object.assign(I18N.en, {
  ...
  pFileEmpty: "The file is empty.", pReadFail: e => `Could not read the file: ${e}`,
...
const Perf = (function () {
  ...
  function readFile(f) {
    ...
    fr.onload = e => {
      try {
        ...
        if (!actualRaw && !targetRaw) return checks([{ t: "err", m: t("pNoHeader") }]);
        ...
        say(t("pReadFail")(err.message), "err");
      }
    };
  ...
  function stage(raw, filename) {
    ...
    for (let i = 1; i < raw.length; i++) {
      ...
      if (!rec.period) {
        // dòng không đọc được kỳ mà cũng không có số nào là dòng tổng hoặc ghi chú
        const hasNums = map.some((f, ci) =>
          f && f !== "period" && f !== "is_forecast" && num(line[ci]) !== 0);
        if (!hasNums) continue;
        bad.push(h + i + 1); continue;
      }
    ...
    if (!out.length) return checks(list.concat([{ t: "err", m: t("pNoValidRows") }]));
  ...
  function targetOnly() {
    checks([{ t: "ok", m: t("pTargetsOnly")(P.stagedTargets.length) }]);
